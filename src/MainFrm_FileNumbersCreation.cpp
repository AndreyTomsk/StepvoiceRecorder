...
static bool IsSuitableForRecording(CString a_filename, DWORD& a_error_code)
{
...
static int GetFileNumber(const CString& fileName, CString& baseName)
{
	//CFileFind ff;

	//someName_01.mp3
	//00 - Prologue.mp3
	//[Leopard-Raws] Barakamon - 01 RAW (NTV 1280x720 x264 AAC).ass
	//The Richest Man In Babylon - 00.mp3
	//Claymore - 01 [DVDRip Dmhy_T3 864x480 x264 AAC].mkv
	//[Leopard-Raws] Gin no Saji (2014) - 01 RAW (CX 1280x720 x264 AAC).ass
	//Mentalist.s01e01.rus(TV3).eng.srt

	//We have here simple implementation. See #1 example in comments above.

	const int dotPos = fileName.ReverseFind(_T('.'));
	const int lowLinePos = fileName.ReverseFind(_T('_'));
	...
	VERIFY(dotPos > lowLinePos);
	const int numberPos = lowLinePos + 1;
	const CString strNumber = fileName.Mid(numberPos, dotPos-numberPos);

	baseName = fileName.Left(lowLinePos);
	return _ttoi(strNumber); //zero if cannot convert
}
//-----------------------------------------------------------------------------

static CString GetNextFilePath(const CString& currentFilePath)
{
	/*
	int fileNumber = 0;
	CString baseName;
    fileNumber = GetFileNumber(_T("someName.mp3"), baseName);
    fileNumber = GetFileNumber(_T("someName_.mp3"), baseName);
    fileNumber = GetFileNumber(_T("someName_00.mp3"), baseName);
    fileNumber = GetFileNumber(_T("someName_01.mp3"), baseName);
    fileNumber = GetFileNumber(_T("someName_000002.mp3"), baseName);
    fileNumber = GetFileNumber(_T("someName_NN.mp3"), baseName);
    fileNumber = GetFileNumber(_T("_.test.mp3"), baseName);
    fileNumber = GetFileNumber(_T("._test.mp3"), baseName);
    fileNumber = GetFileNumber(_T("_"), baseName);
	return;
	*/

	//Returns file name with autoincremented number. Example:
	//"someName.mp3" -> "someName_01.mp3" -> "someName_02.mp3"

	CString fileName, filePath;
	const int slashPos = currentFilePath.ReverseFind(_T('\\'));
	...
	{
		filePath = currentFilePath.Left(slashPos);
		fileName = currentFilePath.Right(currentFilePath.GetLength() - slashPos - 1);
	}

	CString baseName;
	const int fileNumber = GetFileNumber(fileName, baseName);

	CString nextFilePath;
	nextFilePath.Format(_T("%s_%02d.mp3"), baseName, fileNumber+1);
	return nextFilePath;
}
//-----------------------------------------------------------------------------

static CString GetNextRecordingName(const CString& currentFilePath)
{
	//Autocreation of new file name, suitable for recording. Format is:
	//"someName.mp3" -> "someName_01.mp3" -> "someName_02.mp3" -> etc.

	CString filePath = currentFilePath;
	while (true)
	{
		filePath = GetNextFilePath(filePath);
		DWORD errorCode = 0;
		if (IsSuitableForRecording(filePath, errorCode))
			break;
	}
	return filePath;
}
...
void CMainFrame::OnBtnREC()
{
	...
	if (m_record_file.m_hFile == CFile::hFileNull)
	{
		//TODO: autocreation of new file. Format is:
		//  "someName.mp3" -> "someName_01.mp3" -> "someName_02.mp3" -> etc.
		//Were requested from user:
		//  nick_mills_wpslo <wpslo@btinternet.com> (search mail).

		if (g_stream_handle)
		{
			const CString filePath = m_conf.GetConfProg()->strLastFilePath;
			const CString fileName = m_conf.GetConfProg()->strLastFileName;
			const CString fullPath = filePath + _T("\\") + fileName;

			const CString nextRecordingName = GetNextRecordingName(fullPath);
			OpenFile(nextRecordingName);
		}
		else
		{
			OnBtnOPEN();
